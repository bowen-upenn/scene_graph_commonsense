...
def train_local(gpu, args, train_subset, test_subset, faster_rcnn_cfg=None):
    ...
    for epoch in range(args['training']['start_epoch'], args['training']['num_epoch']):
        ...
        for batch_count, data in enumerate(tqdm(train_loader), 0):
            ...
            """
            all_h_subject, all_h_object, all_cat_subject, all_cat_object, all_scat_subject, all_scat_object, \
                all_bbox_subject, all_bbox_object, all_relations_target, all_which_in_batch = None, None, None, None, [], [], None, None, None, None
            ...
            for graph_iter in range(max(num_graph_iter)):
                ...
                for edge_iter in range(graph_iter):
                    ...
                    if all_h_subject is None:
                        all_h_subject, all_h_object, all_cat_subject, all_cat_object, all_scat_subject, all_scat_object, all_bbox_subject, all_bbox_object, all_relations_target, all_which_in_batch = \
                            h_subject, h_object, cat_subject, cat_object, scat_subject, scat_object, bbox_subject, bbox_object, curr_relations_target, which_in_batch
                    else:
                        all_h_subject = torch.cat((all_h_subject, h_subject), dim=0)
                        all_h_object = torch.cat((all_h_object, h_object), dim=0)
                        all_cat_subject = torch.cat((all_cat_subject, cat_subject))
                        all_cat_object = torch.cat((all_cat_object, cat_object))
                        all_scat_subject = all_scat_subject + scat_subject
                        all_scat_object = all_scat_object + scat_object
                        all_bbox_subject = torch.vstack((all_bbox_subject, bbox_subject))
                        all_bbox_object = torch.vstack((all_bbox_object, bbox_object))
                        all_relations_target = torch.cat((all_relations_target, curr_relations_target))
                        all_which_in_batch = torch.cat((all_which_in_batch, which_in_batch))

            assert len(all_h_subject) == len(all_h_object) == len(all_cat_subject) == len(all_cat_object) == len(all_scat_subject) == len(all_scat_object)
            assert len(all_bbox_subject) == len(all_bbox_object) == len(all_relations_target) == len(all_which_in_batch)

            # train the model with all relationships summarized
            loss_connectivity, loss_relationship = 0.0, 0.0
            if args['models']['hierarchical_pred']:
                relation_1, relation_2, relation_3, super_relation, connectivity = edge_head(all_h_subject, all_h_object, all_cat_subject, all_cat_object, all_scat_subject, all_scat_object, rank)
                relation = torch.cat((relation_1, relation_2, relation_3), dim=1)
            else:
                relation, connectivity = edge_head(all_h_subject, all_h_object, all_cat_subject, all_cat_object, all_scat_subject, all_scat_object, rank)
                super_relation = None
            ...
            if args['models']['hierarchical_pred']:
                ...
                if len(connected_3) > 0:
                    loss_relationship += criterion_relationship_3(relation_3[connected_3], all_relations_target[connected_3]

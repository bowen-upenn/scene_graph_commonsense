...
def train_local(gpu, args, train_subset, test_subset, faster_rcnn_cfg=None):
    ...
    for epoch in range(args['training']['start_epoch'], args['training']['num_epoch']):
        ...
        for batch_count, data in enumerate(tqdm(train_loader), 0):
            ...
            all_sub_categories = torch.stack([triplet[0].to(rank) for triplet in all_triplets])
            all_obj_categories = torch.stack([triplet[5].to(rank) for triplet in all_triplets])
            ...
            all_sub_bboxes = [torch.unsqueeze(triplet[2], dim=0).to(rank) for triplet in all_triplets]
            all_obj_bboxes = [torch.unsqueeze(triplet[7], dim=0).to(rank) for triplet in all_triplets]
            ...
            if len(connected) > 0:
                ...
                if args['models']['hierarchical_pred']:
                    ...
                    connected_2 = torch.nonzero(torch.logical_and(relations_target[graph_iter - 1][edge_iter][connected] >= args['models']['num_geometric'],
                                                                  relations_target[graph_iter - 1][edge_iter][connected] < args['models']['num_geometric'] + args['models'][
                                                                      'num_possessive'])).flatten()  # possessive
                    ...
                    if len(connected_2) > 0:
                        loss_relationship += criterion_relationship_2(relation_2[connected][connected_2],
                                                                      relations_target[graph_iter - 1][edge_iter][connected][connected_2] - args['models']['num_geometric'])
                    if len(connected_3) > 0:
                        loss_relationship += criterion_relationship_3(relation_3[connected][connected_3],
                                                                      relations_target[graph_iter - 1][edge_iter][connected][connected_3] - args['models']['num_geometric'] - args['models'][
                                                                          'num_possessive'])
                else:
...
def test_local(args, backbone, edge_head, test_loader, test_record, epoch, rank):
    ...
    with torch.no_grad():
        for batch_count, data in enumerate(tqdm(test_loader), 0):
            ...
            PREPARE INPUT DATA
            """
            images, image_depth, categories, super_categories, masks, bbox, relationships, subj_or_obj = data

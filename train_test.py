        ...
        for batch_count, data in enumerate(tqdm(test_loader), 0):
            ...
            for graph_iter in range(max(num_graph_iter)):
                ...
                for edge_iter in range(graph_iter):
                    ...
                    # print('torch.sigmoid(connectivity[:, 0])', torch.sigmoid(connectivity[:, 0]))
                    hidden = hidden[torch.sigmoid(connectivity[:, 0]) > 0.5]
                    if len(hidden) > 0:
                    ...
                    # print('torch.sigmoid(connectivity[:, 0])', torch.sigmoid(connectivity[:, 0]))
                    hidden2 = hidden2[torch.sigmoid(connectivity[:, 0]) > 0.5]
                    if len(hidden2) > 0:

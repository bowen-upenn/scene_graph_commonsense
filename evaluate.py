...
def inference_single_image(gpu, args, test_subset, faster_rcnn_cfg=None):
    """
...
def eval_pc(gpu, args, test_subset, faster_rcnn_cfg=None):
    ...
    if args['dataset']['dataset'] == 'vg':
        Recall_top3 = Evaluator_PC_Top3(args=args, num_classes=args['models']['num_relations'], iou_thresh=0.5, top_k=[20, 50, 100])
    # Recall_no_graph_constraint = Evaluator_PC_NoGraphConstraint(args=args, num_classes=args['models']['num_relations'], iou_thresh=0.5, top_k=[20, 50, 100])
                    ...
                    if (batch_count % args['training']['eval_freq_test'] == 0) or (batch_count + 1 == len(test_subset)):
                        ...
                        if args['dataset']['dataset'] == 'vg' and args['models']['hierarchical_pred']:
                            Recall_top3.accumulate(which_in_batch, relation, relations_target_directed, super_relation, torch.log(torch.sigmoid(connectivity[:, 0])),
                                                   cat_graph, cat_edge, cat_graph, cat_edge, bbox_graph, bbox_edge, bbox_graph, bbox_edge)
                        # Recall_no_graph_constraint.accumulate(which_in_batch, relation, relations_target_directed, super_relation, torch.log(torch.sigmoid(connectivity[:, 0])),
                        #                   cat_graph, cat_edge, cat_graph, cat_edge, bbox_graph, bbox_edge, bbox_graph, bbox_edge)
            ...
            if (batch_count % args['training']['eval_freq_test'] == 0) or (batch_count + 1 == len(test_loader)):
                if args['dataset']['dataset'] == 'vg':
                    ...
                    if args['models']['hierarchical_pred']:
                        recall_top3, _, mean_recall_top3 = Recall_top3.compute(per_class=True)
                        Recall_top3.clear_data()
                    # recall_ng, _, mean_recall_ng = Recall_no_graph_constraint.compute(per_class=True)
                else:
                    recall, _, mean_recall, _, _, _ = Recall.compute(per_class=True)
                    wmap_rel, wmap_phrase = Recall.compute_precision()
                Recall.clear_data()
                # Recall_no_graph_constraint.clear_data()

            if (batch_count % args['training']['print_freq_test'] == 0) or (batch_count + 1 == len(test_loader)):
                # print("recall_ng", recall_ng, "mean_recall_ng", mean_recall_ng)
                record_test_results(args, test_record, rank, args['training']['test_epoch'], recall_top3, recall, mean_recall_top3, mean_recall, recall_zs, mean_recall_zs,
                                    connectivity_recall, num_connected, num_not_connected, connectivity_precision, num_connected_pred, wmap_rel, wmap_phrase)
...
#             """
#             try:
#                 images, images_s, image_depth, categories_target, super_categories_target, bbox_target, relationships, subj_or_obj = data
#             except:
#                 continue
#
#             if args['models']['detr_or_faster_rcnn'] == 'detr':
...
#
#             if args['models']['detr_or_faster_rcnn'] == 'detr':
#                 bbox_pred = [out_dict['pred_boxes'][i, has_object_pred[i]] for i in range(len(categories_pred)) if torch.sum(has_object_pred[i]) > 0]  # convert from 0-1 to 0-32
...
#
#             print("bbox_pred", len(bbox_pred), [bbox.shape for bbox in bbox_pred])
#             print("categories_pred", len(categories_pred), [len(cat) for cat in categories_pred])
#
#             for i in range(len(categories_pred)):
        ...
        for batch_count, data in enumerate(tqdm(test_loader), 0):
            ...
            for graph_iter in range(max(num_graph_iter)):
                ...
                for edge_iter in range(graph_iter):
                    ...
                    """
                    if args['models']['hierarchical_pred']:
                        relation_1, relation_2, relation_3, super_relation, connectivity = edge_head(h_graph, h_edge, cat_graph_pred, cat_edge_pred,
                                                                                                     scat_graph_pred, scat_edge_pred, rank)
                        relation = torch.cat((relation_1, relation_2, relation_3), dim=1)
                    else:
                        relation, connectivity = edge_head(h_graph, h_edge, cat_graph_pred, cat_edge_pred, scat_graph_pred, scat_edge_pred, rank)
                    ...
                    """
                    if args['models']['hierarchical_pred']:
                        relation_1, relation_2, relation_3, super_relation, connectivity = edge_head(h_edge, h_graph, cat_edge_pred, cat_graph_pred,
                                                                                                     scat_edge_pred, scat_graph_pred, rank)
                        relation = torch.cat((relation_1, relation_2, relation_3), dim=1)
                    else:
                        relation, connectivity = edge_head(h_edge, h_graph, cat_edge_pred, cat_graph_pred, scat_edge_pred, scat_graph_pred, rank)
...
#             """
#             if (batch_count % args['training']['eval_freq_test'] == 0) or (batch_count + 1 == len(test_loader)):
#                 recall, _, mean_recall = Recall.compute(per_class=True)
#                 recall_top3, _, mean_recall_top3 = Recall_top3.compute(per_class=True)
#                 Recall.clear_data()
#                 Recall_top3.clear_data()
#

...
class PostExtractFeatureProcessor(nn.Module):
    """
    From a set of classification scores, box regression and proposals,
    computes the post-processed boxes, and applies NMS to obtain the
    final results
    ...
    def __init__(
        ...
        """
        super(PostExtractFeatureProcessor, self).__init__()
        self.attribute_on = attribute_on
        self.use_gt_box = use_gt_box
        self.later_nms_pred_thres = later_nms_pred_thres
    ...
    def forward(self, x, rel_pair_idxs, boxes, prod_reps):
        ...
        results = []
        for i, (rel_logit, obj_logit, prod_rep, rel_pair_idx, box) in enumerate(zip(
                relation_logits, finetune_obj_logits, prod_reps, rel_pair_idxs, boxes
        )):
            if self.attribute_on:
                att_logit = finetune_att_logits[i]
                att_prob = torch.sigmoid(att_logit)
            ...
            if self.use_gt_box:
                # obj_scores, obj_pred = obj_class_prob[:, 1:].max(dim=1)
                # obj_pred = obj_pred + 1
                obj_scores, obj_pred = obj_class_prob[:, :].max(dim=1)
            else:
                # NOTE: by kaihua, apply late nms for object prediction
                obj_pred = obj_prediction_nms(box.get_field('boxes_per_cls'), obj_logit, self.later_nms_pred_thres)
                obj_score_ind = torch.arange(num_obj_bbox, device=obj_logit.device) * num_obj_class + obj_pred
            ...
            boxlist.add_field('pred_rel_rep', prod_rep.cpu())
...
class ConfidencePostProcessor(nn.Module):
    """
    From a set of classification scores, box regression and proposals,
    computes the post-processed boxes, and applies NMS to obtain the
    final results
    ...
    def __init__(
        ...
        """
        super(ConfidencePostProcessor, self).__init__()
        self.attribute_on = attribute_on
        self.use_gt_box = use_gt_box
        self.later_nms_pred_thres = later_nms_pred_thres

    def forward(self, x, rel_pair_idxs, boxes, confidences):
        """
        Arguments:
            x (tuple[tensor, tensor]): x contains the relation logits
                and finetuned object logits from the relation model.
        ...
        results = []
        for i, (rel_logit, obj_logit, rel_pair_idx, box, confidence) in enumerate(zip(
                relation_logits, finetune_obj_logits, rel_pair_idxs, boxes, confidences
        )):
            if self.attribute_on:
                att_logit = finetune_att_logits[i]
                att_prob = torch.sigmoid(att_logit)
            ...
            triple_scores = rel_scores * obj_scores0 * obj_scores1
            _, sorting_idx = torch.sort(triple_scores.view(-1), dim=0, descending=True)
            boxlist.add_field('unsort_rel_pair_idxs', rel_pair_idx)  # (#rel, 2)
            boxlist.add_field('unsort_pred_rel_scores', rel_class_prob)  # (#rel, #rel_class)
            boxlist.add_field('unsort_pred_rel_labels', rel_class)  # (#rel, )
            boxlist.add_field('unsort_ood_score', confidence)
            rel_pair_idx = rel_pair_idx[sorting_idx]
            rel_class_prob = rel_class_prob[sorting_idx]
            rel_labels = rel_class[sorting_idx]
            scores = confidence[sorting_idx]
            boxlist.add_field('rel_pair_idxs', rel_pair_idx)  # (#rel, 2)
            boxlist.add_field('pred_rel_scores', rel_class_prob)  # (#rel, #rel_class)
            boxlist.add_field('pred_rel_labels', rel_labels)  # (#rel, )
            boxlist.add_field('ood_score', scores)
            # should have fields : rel_pair_idxs, pred_rel_class_prob, pred_rel_labels, pred_labels, pred_scores
            # Note
            # TODO Kaihua: add a new type of element, which can have different length with boxlist (similar to field, except that once
            # the boxlist has such an element, the slicing operation should be forbidden.)
    ...
    def forward(self, x, rel_pair_idxs, boxes):
        ...
        Returns:
            results (list[BoxList]): one BoxList for each image, containing
                the extra fields labels and scores
        """
        # different from rel_logits in PostProcessor, rel1_probs, rel2_probs, rel3_probs have been applied softmax
...
def make_roi_relation_post_extract_feature_processor(cfg):
    attribute_on = cfg.MODEL.ATTRIBUTE_ON
    use_gt_box = cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX
    later_nms_pred_thres = cfg.TEST.RELATION.LATER_NMS_PREDICTION_THRES

    postprocessor = PostExtractFeatureProcessor(
        attribute_on,
        use_gt_box,
        later_nms_pred_thres,
    )
...
def make_roi_relation_confidence_post_processor(cfg):
    attribute_on = cfg.MODEL.ATTRIBUTE_ON
    use_gt_box = cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX
    later_nms_pred_thres = cfg.TEST.RELATION.LATER_NMS_PREDICTION_THRES

    postprocessor = ConfidencePostProcessor(
        attribute_on,
        use_gt_box,
        later_nms_pred_thres,
    )

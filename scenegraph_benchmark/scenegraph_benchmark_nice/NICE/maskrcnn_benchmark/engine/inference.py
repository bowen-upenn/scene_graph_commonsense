...
def ood_compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None):
    model.eval()
    results_dict = {}
    cpu_device = torch.device("cpu")
    torch.cuda.empty_cache()
    ...
    for img_id in results_dict.keys():
        # print(results_dict[img_id])
        # print(results_dict[img_id].get_field('ood_score'))
        ave_scores_list.extend(results_dict[img_id].get_field('ood_score').data)
    torch.cuda.empty_cache()
    return results_dict, np.average(ave_scores_list)
...
def inference(
    ...
    if load_prediction_from_cache:
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
            predictions, ave_score = ood_compute_on_dataset(model, data_loader, device, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER, timer=inference_timer)
        else:
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
        logger.info("Average OOD Score: {})".format(ave_score))
    logger.info(

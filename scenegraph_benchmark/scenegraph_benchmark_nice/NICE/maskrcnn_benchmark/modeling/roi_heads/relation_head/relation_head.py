...
from .inference import make_roi_relation_post_processor, make_roi_relation_post_extract_feature_processor, make_roi_relation_confidence_post_processor
from .loss import make_roi_relation_loss_evaluator, make_roi_confidence_loss_evaluator
...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.loss_confidence_evaluator = make_roi_confidence_loss_evaluator(cfg)
        self.samp_processor = make_roi_relation_samp_processor(cfg)
        self.post_confidence_processor = make_roi_relation_confidence_post_processor(cfg)
        self.post_extract_feature_processor = make_roi_relation_post_extract_feature_processor(cfg)
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.training:
            ...
            with torch.no_grad():
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    if not self.cfg.ADD_BG and self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
                        proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_non_bg_relsample(proposals, targets)
                    else:
            ...
            if self.cfg.TYPE == 'complete_bg' and self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
                proposals, rel_labels, rel_pair_idxs = self.samp_processor.prepare_just_bg_test_pairs(features[0].device, proposals, targets)
            elif self.cfg.TYPE == 'extract_pos':
                proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_non_bg_relsample(proposals, targets)
            else:
                rel_pair_idxs = self.samp_processor.prepare_test_pairs(features[0].device, proposals)

        if len(rel_pair_idxs) == 0:
            return None, None, {}
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "MotifHierarchicalPredictor" or \
            ...
            # final classifier that converts the features into predictions
            # should corresponding to all the functions and layers after the self.context class
            if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
                refine_logits, relation_logits, add_losses, prod_reps, confidences = self.predictor(proposals, rel_pair_idxs, rel_labels,
                                                                                rel_binarys, roi_features, union_features,
                                                                                logger)
            elif self.cfg.TYPE == 'extract_pos':
                refine_logits, relation_logits, add_losses, prod_reps = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys,
                                                                        roi_features, union_features, logger)
            else:
                refine_logits, relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys,
                                                                        roi_features, union_features, logger)

            # for test
            if not self.training:
                if self.cfg.TYPE == 'extract_pos':
                    result = self.post_extract_feature_processor((relation_logits, refine_logits), rel_pair_idxs, proposals, prod_reps)
                elif self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
                    result = self.post_confidence_processor((relation_logits, refine_logits), rel_pair_idxs, proposals, confidences)
                else:
                    result = self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals)
                return roi_features, result, {}

            if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifConfidencePredictor':
                loss_relation, loss_refine = self.loss_confidence_evaluator(proposals, rel_labels, relation_logits,
                                                                            refine_logits, confidences)
            else:

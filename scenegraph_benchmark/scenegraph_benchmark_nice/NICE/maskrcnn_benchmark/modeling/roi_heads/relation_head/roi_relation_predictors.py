...
from .model_motifs_hierarchical import BayesHead, BayesHeadProb
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.type == 'extract_pos':
            prod_rep = prod_rep.split(num_rels, dim=0)
            return obj_dists, rel_dists, add_losses, prod_rep
        if self.attribute_on:
            att_dists = att_dists.split(num_objs, dim=0)
            return (obj_dists, att_dists), rel_dists, add_losses
        else:
...
@registry.ROI_RELATION_PREDICTOR.register("MotifConfidencePredictor")
class MotifConfidencePredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifConfidencePredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = 150
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        # self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.num_rel_cls = 50
        ...
        self.post_cat = nn.Linear(self.hidden_dim * 2, self.pooling_dim)
        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls, bias=True)
        self.confidence = nn.Linear(self.pooling_dim, 1)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        rel_dists = self.rel_compress(prod_rep)
        confidences = self.confidence(prod_rep)
        ...
        obj_dists = obj_dists.split(num_objs, dim=0)
        rel_dists = rel_dists.split(num_rels, dim=0)
        confidences = confidences.split(num_rels, dim=0)
        ...
        if self.attribute_on:
            ...
            return obj_dists, rel_dists, add_losses, prod_rep, confidences

...
class RelationConfidenceLossComputation(object):
    """
    Computes the loss for relation triplet.
    Also supports FPN
    """
    ...
    def __init__(
        ...
        self.use_label_smoothing = use_label_smoothing
        # self.pred_weight = (1.0 / torch.FloatTensor([0.5,] + predicate_proportion)).cuda()
        self.pred_weight = (1.0 / torch.FloatTensor(predicate_proportion)).cuda()
        # in_features = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        if self.use_label_smoothing:
            self.criterion_loss = Label_Smoothing_Regression(e=0.01)
        else:
            self.criterion_loss = nn.CrossEntropyLoss()
            # self.prediction_criterion = nn.NLLLoss()
            self.prediction_criterion = nn.NLLLoss(weight=self.pred_weight)
        self.lmbda = 0.1


    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, confidences):
        """
        Computes the loss for relation triplet.
        This requires that the subsample method has been called beforehand.
        ...
        fg_labels = cat([proposal.get_field("labels") for proposal in proposals], dim=0)
        rel_labels = cat(rel_labels, dim=0)
        confidences = cat(confidences, dim=0)
        # rel_labels = Variable(rel_labels).cuda(async=True)
        labels_onehot = Variable(encode_onehot(rel_labels, 50))

        pred_originals = F.softmax(relation_logits, dim=-1)
        # confidences = F.sigmoid(confidences)

        eps = 1e-12

        pred_originals = torch.clamp(pred_originals, 0. + eps, 1. - eps)
        confidences = torch.clamp(confidences, 0. + eps, 1. - eps)

        # Randomly set half of the confidences to 1 (i.e. no hints)
        b = Variable(torch.bernoulli(torch.Tensor(confidences.size()).uniform_(0, 1))).cuda()
        conf = confidences * b + (1 - b)
        pred_new = pred_originals * conf.expand_as(pred_originals) + labels_onehot * (
                    1 - conf.expand_as(labels_onehot))
        pred_new = torch.log(pred_new)

        xentropy_loss = self.prediction_criterion(pred_new, rel_labels)
        confidence_loss = torch.mean(-torch.log(confidences))


        loss_refine_obj = self.criterion_loss(refine_obj_logits, fg_labels.long())
        loss_relation = xentropy_loss + (self.lmbda * confidence_loss)
...
def make_roi_confidence_loss_evaluator(cfg):

    loss_evaluator = RelationConfidenceLossComputation(
        cfg.MODEL.ATTRIBUTE_ON,
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES,
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.MAX_ATTRIBUTES,
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.ATTRIBUTE_BGFG_SAMPLE,

...
#CUDA_VISIBLE_DEVICES=1,2 python -m torch.distributed.launch --master_port 10091 --nproc_per_node=2 \
...
#CUDA_LAUNCH_BLOCKING=1
CUDA_VISIBLE_DEVICES=1,2 python -m torch.distributed.launch --master_port 10081 --nproc_per_node=2 \
  tools/relation_train_net.py --config-file "configs/sup-50.yaml" \
  MODEL.ROI_RELATION_HEAD.USE_GT_BOX True MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL True \
  ...
  GLOVE_DIR $OUTPATH/glove \
  MODEL.PRETRAINED_DETECTOR_CKPT $OUTPATH/pretrained_faster_rcnn/model_final.pth  \
  OUTPUT_DIR $OUTPATH/motif-ietrans \
  TEST.METRIC "R"
